...
class CompletionProposalPopup implements IContentAssistListener {
	...
	final class ProposalSelectionHandler extends AbstractHandler {
		...
		public ProposalSelectionHandler(int operationCode) {
			Assert.isLegal(operationCode == SELECT_NEXT
					|| operationCode == SELECT_PREVIOUS);
	 ...
	 * @since 3.2
	 */
	private static final class EmptyProposal implements ICompletionProposal,
	...
	private final Runnable fFilterRunnable = new Runnable() {
		public void run() {
			...
			if (!Helper.okToUse(fContentAssistSubjectControlAdapter
					.getControl()))
				return;
			...
			try {
				if (offset > -1) {
					DocumentEvent event = TextUtilities
							.mergeProcessedDocumentEvents(fDocumentEvents);
	...
	/**
	 * The sorter to be used for sorting the proposals or <code>null</code> if
	 * no sorting is requested.
	 * 
	 * @since 3.8
	 */
	...
	public CompletionProposalPopup(ContentAssistant contentAssistant,
			ITextViewer viewer, AdditionalInfoController infoController) {
		fContentAssistant = contentAssistant;
		fViewer = viewer;
		fAdditionalInfoController = infoController;
		fContentAssistSubjectControlAdapter = new ContentAssistSubjectControlAdapter(
				fViewer);
	}
	...
	public String showProposals(final boolean autoActivated) {
		...
		final Control control = fContentAssistSubjectControlAdapter
				.getControl();

		if (!Helper.okToUse(fProposalShell) && control != null
				&& !control.isDisposed()) {
			...
			BusyIndicator.showWhile(control.getDisplay(), new Runnable() {
				public void run() {
					...
					fInvocationOffset = fContentAssistSubjectControlAdapter
							.getSelectedRange().x;
					fFilterOffset = fInvocationOffset;
					fLastCompletionOffset = fFilterOffset;
					fComputedProposals = computeProposals(fInvocationOffset);

					int count = (fComputedProposals == null ? 0
							: fComputedProposals.length);
					if (count == 0 && hideWhenNoProposals(autoActivated))
						return;

					if (count == 1 && !autoActivated
							&& canAutoInsert(fComputedProposals[0])) {
						insertProposal(fComputedProposals[0], (char) 0, 0,
								fInvocationOffset);
						hide();
					} else {
	...
	private boolean hideWhenNoProposals(boolean autoActivated) {
		if (autoActivated || !fContentAssistant.isShowEmptyList()) {
			if (!autoActivated) {
				Control control = fContentAssistSubjectControlAdapter
						.getControl();
	...
	private ICompletionProposal[] computeProposals(int offset) {
		if (fContentAssistSubjectControl != null)
			return fContentAssistant.computeCompletionProposals(
					fContentAssistSubjectControl, offset);
	...
	private void createProposalSelector() {
		...
		if (USE_VIRTUAL) {
			fProposalTable = new Table(fProposalShell, SWT.H_SCROLL
		...
		fIsColoredLabelsSupportEnabled = fContentAssistant
		...
		fContentAssistant.addToLayout(this, fProposalShell,
				ContentAssistant.LayoutManager.LAYOUT_PROPOSAL_SELECTOR,
		...
		fProposalShell.addControlListener(new ControlListener() {
			...
			public void controlResized(ControlEvent e) {
				if (fAdditionalInfoController != null) {
					...
					fAdditionalInfoController.setSizeConstraints(50, 10, true,
							false);
		...
		fProposalShell.setBackground(control.getDisplay().getSystemColor(
				SWT.COLOR_GRAY));
	...
	int getMinimalHeight() {
		...
		if (Helper.okToUse(fProposalTable)) {
			...
			Rectangle trim = fProposalTable.computeTrim(0, 0, SWT.DEFAULT,
					items);
			height = trim.height;
		}
	...
	private void addCommandSupport(final Control control) {
		final KeySequence commandSequence = fContentAssistant
				.getRepeatedInvocationKeySequence();
		if (commandSequence != null && !commandSequence.isEmpty()
				&& fContentAssistant.isRepeatedInvocationMode()) {
				...
				public void focusGained(FocusEvent e) {
					if (Helper.okToUse(control)) {
						if (fTraverseListener == null) {
							fTraverseListener = new TraverseListener() {
								public void keyTraversed(TraverseEvent event) {
									if (event.detail == SWT.TRAVERSE_TAB_NEXT) {
										IInformationControl iControl = fAdditionalInfoController
												.getCurrentInformationControl2();
										if (fAdditionalInfoController
												.getInternalAccessor()
												.canReplace(iControl)) {
											fAdditionalInfoController
													.getInternalAccessor()
							...
							fProposalTable
									.addTraverseListener(fTraverseListener);
						}
					}
				}
	...
	private Color getBackgroundColor(Control control) {
		...
		if (c == null)
			c = JFaceResources.getColorRegistry().get(
	...
	private void createMessageText() {
		if (fMessageText == null) {
			...
			if (fContentAssistant.isRepeatedInvocationMode()) {
				fMessageText.setCursor(fProposalShell.getDisplay()
	...
	private void handleSetData(Event event) {
		...
		if (0 <= index && index < fFilteredProposals.length) {
			...
			if (fIsColoredLabelsSupportEnabled
				...
				StyledString styledString = ((ICompletionProposalExtension6) current)
	...
	private ICompletionProposal getSelectedProposal() {
		...
		if (fFilteredProposals == null || i < 0
				|| i >= fFilteredProposals.length)
			return null;
	...
	private void insertSelectedProposalWithMask(int stateMask) {
		...
		hide();
		if (p != null)
			insertProposal(p, (char) 0, stateMask,
	...
	private void insertProposal(ICompletionProposal p, char trigger,
		...
		IEditingSupport helper = new IEditingSupport() {
			...
			public boolean isOriginator(DocumentEvent event, IRegion focus) {
				return focus.getOffset() <= offset
						&& focus.getOffset() + focus.getLength() >= offset;
			}
		...
		try {
			...
			IDocument document = fContentAssistSubjectControlAdapter
					.getDocument();
			...
			if (selection != null) {
				fContentAssistSubjectControlAdapter.setSelectedRange(
						selection.x, selection.y);
				fContentAssistSubjectControlAdapter.revealRange(selection.x,
						selection.y);
			}
			...
			if (info != null) {
				...
				if (p instanceof ICompletionProposalExtension) {
					ICompletionProposalExtension e = (ICompletionProposalExtension) p;
					contextInformationOffset = e
							.getContextInformationPosition();
				} else {
					if (selection == null)
						selection = fContentAssistSubjectControlAdapter
								.getSelectedRange();
	...
	public boolean hasFocus() {
		if (Helper.okToUse(fProposalShell)) {
			 ...
			 */
			if (fAdditionalInfoController != null) {
				IInformationControl informationControl = fAdditionalInfoController
						.getCurrentInformationControl2();
				if (informationControl != null
						&& informationControl.isFocusControl())
					return true;
				InformationControlReplacer replacer = fAdditionalInfoController
						.getInternalAccessor().getInformationControlReplacer();
				if (replacer != null) {
					informationControl = replacer
							.getCurrentInformationControl2();
					if (informationControl != null
							&& informationControl.isFocusControl())
						return true;
	...
	public void hide() {
		...
		if (Helper.okToUse(fProposalShell)) {
			...
			fContentAssistant.removeContentAssistListener(this,
					ContentAssistant.PROPOSAL_SELECTOR);
	...
	private void unregister() {
		...
		if (fKeyListener != null
				&& fContentAssistSubjectControlAdapter.getControl() != null
				&& !fContentAssistSubjectControlAdapter.getControl()
						.isDisposed()) {
	...
	/**
	 * Initializes the proposal selector with these given proposals. If a
	 * proposal sorter is configured, the given proposals are sorted before.
	 * 
	 * @param proposals
	 *            the proposals
	...
	private void setProposals(ICompletionProposal[] proposals,
		...
		if (Helper.okToUse(fProposalTable)) {
			if (oldProposal instanceof ICompletionProposalExtension2
					&& fViewer != null)
				((ICompletionProposalExtension2) oldProposal)
						.unselected(fViewer);
	...
	private Point getLocation() {
		...
		Rectangle location = fContentAssistant.getLayoutManager()
				.computeBoundsBelowAbove(fProposalShell,
						fSize == null ? fProposalShell.getSize() : fSize,
						caret, this);
	...
	private void displayProposals() {
		...
		if (fContentAssistant.addContentAssistListener(this,
				ContentAssistant.PROPOSAL_SELECTOR)) {
	...
	public boolean verifyKey(VerifyEvent e) {
		...
		if (key == 0) {
			...
			int visibleRows = (fProposalTable.getSize().y / fProposalTable
					.getItemHeight()) - 1;
			int itemCount = fProposalTable.getItemCount();
			switch (e.keyCode) {
	 ...
	 * @since 3.2
	 */
	private void fireSelectionEvent(ICompletionProposal proposal,
			boolean smartToggle) {
		fContentAssistant.fireSelectionEvent(proposal, smartToggle);
	}

	/**
	 * Fires an event after applying the given proposal, see
	 * {@link ICompletionListenerExtension2}.
	 * 
	 * @param proposal
	...
	private ICompletionProposal[] computeFilteredProposals(int offset,
		...
		return (ICompletionProposal[]) filtered
		...
		if (Helper.okToUse(fProposalShell) && fFilteredProposals != null) {
			...
			BusyIndicator.showWhile(control.getDisplay(), new Runnable() {
				public void run() {
					...
					int count = (fFilteredProposals == null ? 0
							: fFilteredProposals.length);
					if (count == 0 && hideWhenNoProposals(false))
						return;

					if (count == 1 && canAutoInsert(fFilteredProposals[0])) {
						insertProposal(fFilteredProposals[0], (char) 0, 0,
								fInvocationOffset);
						hide();
					} else {
						ensureDocumentListenerInstalled();
	...
	private boolean completeCommonPrefix() {
		...
		for (int i = 0; i < fFilteredProposals.length; i++) {
			...
			int start = ((ICompletionProposalExtension3) proposal)
					.getPrefixCompletionStart(
							fContentAssistSubjectControlAdapter.getDocument(),
							fFilterOffset);
			CharSequence insertion = ((ICompletionProposalExtension3) proposal)
					.getPrefixCompletionText(
							fContentAssistSubjectControlAdapter.getDocument(),
							fFilterOffset);
			if (insertion == null)
				insertion = TextProcessor
						.deprocess(proposal.getDisplayString());
			try {
				int prefixLength = fFilterOffset - start;
				int relativeCompletionOffset = Math.min(insertion.length(),
						prefixLength);
				String prefix = document.get(start, prefixLength);
				if (!isWrongCaseMatch
						&& insertion.toString().startsWith(prefix)) {
					isWrongCaseMatch = false;
					rightCase.add(proposal);
					CharSequence newPostfix = insertion.subSequence(
							relativeCompletionOffset, insertion.length());
					if (rightCasePostfix == null)
						rightCasePostfix = new StringBuffer(
								newPostfix.toString());
					else
						truncatePostfix(rightCasePostfix, newPostfix);
				} else if (i == 0 || isWrongCaseMatch) {
					CharSequence newPrefix = insertion.subSequence(0,
							relativeCompletionOffset);
					if (isPrefixCompatible(wrongCasePrefix,
							wrongCasePrefixStart, newPrefix, start, document)) {
						...
						if (wrongCasePostfix == null)
							wrongCasePostfix = new StringBuffer(
									newPostfix.toString());
						else
			...
			if (rightCasePostfix != null && rightCasePostfix.length() == 0
					&& rightCase.size() > 1)
				return false;
		}
		...
		if (rightCase.size() == 1) {
			ICompletionProposal proposal = (ICompletionProposal) rightCase
					.get(0);
			...
			ICompletionProposal proposal = (ICompletionProposal) wrongCase
					.get(0);
		...
		try {
			...
			int to = Math.min(document.getLength(),
					fFilterOffset + postfix.length());
			StringBuffer inDocument = new StringBuffer(document.get(
					fFilterOffset, to - fFilterOffset));
			truncatePostfix(inDocument, postfix);

			// 5: replace and reveal
			document.replace(fFilterOffset - prefix.length(), prefix.length()
					+ inDocument.length(),
					prefix.toString() + postfix.toString());

			fContentAssistSubjectControlAdapter.setSelectedRange(fFilterOffset
					+ postfix.length(), 0);
			fContentAssistSubjectControlAdapter.revealRange(fFilterOffset
					+ postfix.length(), 0);
			fFilterOffset += postfix.length();
			fLastCompletionOffset = fFilterOffset;
	 ...
	 * @since 3.1
	 */
	private boolean isPrefixCompatible(CharSequence oneSequence, int oneOffset,
			CharSequence twoSequence, int twoOffset, IDocument document)
		...
		String one = document.get(oneOffset, min - oneOffset)
				+ oneSequence
				+ document.get(oneEnd,
						Math.min(fFilterOffset, fFilterOffset - oneEnd));
		String two = document.get(twoOffset, min - twoOffset)
				+ twoSequence
				+ document.get(twoEnd,
						Math.min(fFilterOffset, fFilterOffset - twoEnd));
	 ...
	 * 
	 * @param sorter
	 *            the sorter to be used, or <code>null</code> if no sorting is
	 *            requested
	 * @since 3.8
	 ...
	 * 
	 * @param proposals
	 *            the new proposals to display in the popup window
	 * @throws NullPointerException
	 *             if no sorter has been set
	 * @since 3.8
	 */
